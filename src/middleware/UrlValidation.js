...
const createUrlValidation = async function (req, res, next) {
    ...
    if (isValidData(longUrl.trim()) != true)
    {
        let str= "longUrl " + isValidData(longUrl.trim())
        //console.log(str);
        return res.status(400).send({ status: false, msg: str });
    }
    if (!isUrl(longUrl.trim())) return res.status(400).send({ status: false, message: "Invalid format!! enter a valid URL" });
